...
const StoreOwner = () => {
  const { token } = useAuth();
  const { execute, data: storeOwnerData } = useService(StoreOwnerService.getAll);
  const updateOwnerProfile = useService(StoreOwnerService.store);
  const [storeOwnerForm] = Form.useForm();
  const { success, error } = useNotification();

  const fetchStoreOwnerProfile = useCallback(() => {
    execute({ token });
  }, [execute, token]);
  ...
  function handleValuesChange(changedValue) {
    setRealtimeData((prev) => ({ ...prev, ...changedValue }));
  }
  ...
  return (
    <Form
      ...
      onFinish={async (values) => {
        const { message, isSuccess } = await updateOwnerProfile.execute({ ...values }, token, values.logo.file);
        if (isSuccess) {
